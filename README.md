...
- **代码片段导入**：支持从剪切板自动识别并导入 curl、fetch、wget
- **代码片段导出**：支持选择 curl、fetch、wget 后直接复制到剪切板
- **智能合并**：
...
- ✨ 编辑界面默认展示上一次响应结果记录，便于查看接口状态；
- ✨ api列表面板增加请求状态显示，便于查看接口状态；
- ✨ 编辑界面增加响应结果显隐控制；
...
- ✨ `api`编辑页面支持从剪切板自动识别`curl`、`fetch`、`wget`代码，并支持导出为相应代码片段
